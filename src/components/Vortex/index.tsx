...
const BASE_ZOOM = 67;
const fovForZoom = (zoom: number) => Math.min(Math.max(2 * BASE_ZOOM - zoom, 1), 175);
...
function monotone(points: [number, number][]) {
  ...
  const slope: number[] = [];
  for (let i = 0; i < n - 1; i++) slope[i] = (points[i + 1][1] - points[i][1]) / (points[i + 1][0] - points[i][0]);
  ...
  return (x: number) => {
    ...
    const h = points[i + 1][0] - points[i][0], t = (x - points[i][0]) / h;
    const t2 = t * t, t3 = t2 * t;
    return (2 * t3 - 3 * t2 + 1) * points[i][1] + (t3 - 2 * t2 + t) * h * m[i] + (-2 * t3 + 3 * t2) * points[i + 1][1] + (t3 - t2) * h * m[i + 1];
  };
}
...
function makeShape(cfg: Record<string, number>) {
  ...
  return {
    writePoint(out: Float32Array, at: number, s: number, lane: number, flow: number, wobble: number, phase: number, time: number) {
      ...
      out[at] = Math.cos(a) * rr; out[at + 1] = y; out[at + 2] = Math.sin(a) * rr;
    },
...
function createVortex(canvas: HTMLCanvasElement, container: HTMLElement, cfgRef: { current: Record<string, unknown> }): VortexAPI {
  ...
  const withRepel = <T extends THREE.Material>(material: T): T => {
    material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader
        .replace("void main() {", `uniform vec2 uMouse;uniform float uAspect;uniform float uRadius;uniform float uStrength;void main() {`)
        .replace("#include <fog_vertex>", `#include <fog_vertex>\nif(uStrength>0.0&&uRadius>0.0&&gl_Position.w>0.0){vec2 ndc=gl_Position.xy/gl_Position.w;vec2 off=ndc-uMouse;float dist=length(off*vec2(uAspect,1.0));float f=uStrength*exp(-(dist*dist)/(2.0*uRadius*uRadius));float m=length(off);if(m>1e-4){ndc+=(off/m)*f;gl_Position.xy=ndc*gl_Position.w;}}`);
    };
    return material;
  };
  ...
  let strands: Strand[] = [];
  let strandPos = new Float32Array(0), strandCol = new Float32Array(0);
  let strandGeo: THREE.BufferGeometry | null = null;
  let dotList: Dot[] = [], dotCount = 0;
  let dotMesh: THREE.InstancedMesh | null = null;
  let dotColors = new Float32Array(0), dotHome = new Float32Array(0);
  let dotShift = new Float32Array(0), dotVel = new Float32Array(0);
  let dotScale = new Float32Array(0), dotScaleVel = new Float32Array(0);
  let dotHitAt = new Float32Array(0), dotFlash = new Float32Array(0), dotAlive = new Float32Array(0);
  ...
  function syncColors(cfg: Record<string, unknown>) {
    if (cfg.lineColor !== lastColors.line) { tint.strand.set(cfg.lineColor as string); lastColors.line = cfg.lineColor as string; }
  ...
  function build() {
    ...
    disposables = [];
    shape = makeShape(cfg as Record<string, number>);
    camera.fov = fovForZoom(cfg.zoom as number);
    camera.updateProjectionMatrix();
    syncColors(cfg);

    const count = Math.max(3, Math.round(cfg.lineCount as number));
    const segs = STRAND_SEGMENTS - 1;
    const verts = count * segs * 2;
    strandPos = new Float32Array(verts * 3); strandCol = new Float32Array(verts * 3);
    ...
    const strandMat = track(withRepel(new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending, depthWrite: false })));
    const strandLines = new THREE.LineSegments(strandGeo, strandMat);
    strandLines.frustumCulled = false; group.add(strandLines);
    ...
    if (dotCount > 0) {
      ...
      (dotMesh.instanceColor as THREE.InstancedBufferAttribute).setUsage(THREE.DynamicDrawUsage);
      dotMesh.frustumCulled = false; group.add(dotMesh);
    } else { dotMesh = null; }
  ...
  return {
    ...
    dispose() {
      ...
      for (const d of disposables) d.dispose();
      renderer.dispose();
      if ((renderer as THREE.WebGLRenderer & { forceContextLoss?: () => void }).forceContextLoss) {
...
export default function Vortex(props: VortexProps) {
  ...
  const buildKey = JSON.stringify([
    config.floorRadius, config.waistRadius, config.crownRadius, config.waistAt,
    config.twist, config.lineCount, config.showDots, config.dotCount,
    config.showComets, config.cometCount, config.cometTail,
  ]);

  const configRef = React.useRef(config);
  configRef.current = config;
  const apiRef = React.useRef<(VortexAPI & { canvas: HTMLCanvasElement }) | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (!apiRef.current) {
      ...
      try {
        const api = createVortex(canvas, container, configRef as React.MutableRefObject<Record<string, unknown>>);
        apiRef.current = { ...api, canvas };
      } catch (err) {
    ...
    return () => {
      if (apiRef.current) {
        ...
        if (apiRef.current.canvas.parentNode) {
          apiRef.current.canvas.parentNode.removeChild(apiRef.current.canvas);
        }
        apiRef.current = null;
      }
    };
  ...
  return (
    <div
      ref={containerRef}
      className={className}
      style={{ position: "relative", background, overflow: "hidden", ...style }}
    >
      {/* Canvas is injected dynamically via useEffect to survive React 18 Strict Mode double-invocations */}
    </div>
  );
}
